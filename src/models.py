...
def scale_features(X_train, X_val, X_test) -> tuple:
    """StandardScaler fiteado SOLO sobre X_train, aplicado a val y test.

    Retorna: (X_train_scaled, X_val_scaled, X_test_scaled, scaler)
    El scaler se devuelve para guardarlo y usarlo en predict.py sobre el test set externo.
    """
...
def train_logistic_regression(X_train, y_train) -> LogisticRegression:
    ...
    compensando el leve desbalance sin necesidad de resampling.
    """
...
def train_xgboost(X_train, y_train, X_val, y_val) -> XGBClassifier:
    ...
    scale_pos_weight = n_negativos / n_positivos — equivalente a class_weight en sklearn.
    Usa early stopping sobre val para evitar overfitting: para si el AUC no mejora en 50 rondas.
    """
...
def train_mlp(X_train, y_train, X_val, y_val) -> MLPClassifier:
    ...
    early_stopping=True reserva el 10% de X_train para validación interna de sklearn,
    pero al pasarle X_val directamente al constructor no es posible en sklearn puro.
    Usamos validation_fraction=0 y monitoreamos con el X_val externo para reproducibilidad.
    """
...
def evaluate_model(model, X, y, split_name: str = "test") -> dict:
    """Evalúa un modelo y retorna métricas como dict.

    Métricas: accuracy, f1 (weighted), roc_auc, precision (weighted), recall (weighted).
    """
...
def load_model(model_name: str, models_dir: str) -> tuple:
    """Carga modelo y scaler guardados con joblib. Retorna (model, scaler)."""
...
def shap_analysis(xgb_model, X_train, X_test, feature_names: list) -> None:
    ...
    Produce:
    1. Summary plot — importancia global de cada feature (beeswarm)
    2. Waterfall plots — explicación de 3 predicciones individuales del test set:
       una correcta, un falso positivo y un falso negativo
    """
